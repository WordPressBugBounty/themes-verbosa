...
function verbosa_custom_excerpt_more() {
	if ( ! is_attachment() ) {
		echo wp_kses_post( verbosa_continue_reading_link() );
	}
}
...
function verbosa_continue_reading_link() {
	...
	return '<a class="continue-reading-link" href="'. esc_url( get_permalink() ) . '"><span>' . wp_kses_post( $verbosa_excerptcont ) . '</span><em class="screen-reader-text">"' . get_the_title() . '"</em><i class="icon-arrow-right2"></i></a>';
}

add_filter( 'the_content_more_link', 'verbosa_continue_reading_link' );
...
function verbosa_posted_author() {
	...
	if ( $verbosa_meta_author ) {
		echo sprintf(
			...
			sprintf( esc_attr__( 'View all posts by %s', 'verbosa' ), esc_attr( get_the_author() ) ),
			get_the_author()
		);
	}
 ...
 * Posted date/time
 */
if ( ! function_exists( 'verbosa_posted_date' ) ) :
function verbosa_posted_date() {
	...
	if ( $verbosa_meta_date || $verbosa_meta_time ) {
		$date = ''; $time = '';
		if ( $verbosa_meta_date ) { $date = esc_html( get_the_date() ); }
		if ( $verbosa_meta_time ) { $time = esc_html( get_the_time() ); }

		?>

		<span class="onDate date" >
			<i class="icon-calendar icon-metas" title="<?php esc_attr_e( "Date", "verbosa" ) ?>"></i>
			<time class="published" datetime="<?php echo esc_attr( get_the_time( 'c' ) ) ?>" <?php cryout_schema_microdata('time') ?>><?php echo $date . ( ( $verbosa_meta_date && $verbosa_meta_time ) ? ', ' : '' ) . $time ?></time>
 ...
 * Posted tags
 */
if ( ! function_exists( 'verbosa_posted_tags' ) ) :
 ...
 */
if ( ! function_exists( 'verbosa_posted_edit' ) ) :
function verbosa_posted_edit() {
...
function verbosa_meta_infos() {
	...
	add_action( 'cryout_post_meta_hook', 'verbosa_posted_edit', 99 );
	...
	if ( ! empty( $featured_image[0] ) ):
		...
		<div class="post-thumbnail-container" <?php cryout_schema_microdata( 'image' ); ?>>
			<a href="<?php echo esc_url( get_permalink( $post->ID ) ) ?>" title="<?php echo esc_attr( get_post_field( 'post_title', $post->ID ) ) ?>"
				<?php cryout_echo_bgimage( $featured_image_url, 'post-featured-image' ) ?> tabindex="-1">
			</a>
